...
├── vendapronta-pdv/          # SaaS multi-empresa para equipes de venda (PDV)
...
├── estoque-aberto/           # SaaS de controle de faltas de estoque (farmácia)
...
Cinco dos seis projetos (PRECIFÁCIL, VENDAPRONTA PDV, Encanto Decorações, Corretora de Milhões e Estoque Aberto) foram construídos originalmente para uso real, com dados de negócio reais: clientes, preços, contratos, empresas. Para publicar aqui como portfólio:
...
- **Sem backend**: projetos que originalmente dependiam de banco de dados (Supabase, Postgres) ou de um framework server-side (Django) foram adaptados para rodar 100% no navegador, com os dados guardados em `localStorage`. Nenhuma dessas cinco demonstrações depende de servidor, chave de API ou conexão externa.
